...
constVerwerkteVerwijzingDir = (
    base_dir + "/Verwerkteverwijzingen"
)
...
def getActualImageUrlFromImx(driver):
    ...
    if len(doc) > 0:
        ...
        match = regex.findall(
            patroon_href_verwijzing_plaatje, driver.page_source, regex.IGNORECASE
        )
        if len(match) > 0:
            return match[0]
    return None
...
def getActualImageUrlFromTurboimagehost(driver):
    ...
    if len(doc) > 0:
        doc = doc[0]
        match = regex.findall(
            patroon_src_verwijzing_plaatje, driver.page_source, regex.IGNORECASE
        )
        if len(match) > 0:
            return match[0]
    return None
...
def plaatje_gedownload(url, doelDir):
    ...
    if len(organisatie) == 1:
        ...
        img_url = None
        if organisatie == "imx":
            img_url = getActualImageUrlFromImx(browser)
        if organisatie == "turboimagehost":
            img_url = getActualImageUrlFromTurboimagehost(browser)
...
opTePakkenVerwijzingDirs = [
    d
    for d in os.listdir(constVerwijzingDir)
    if os.path.isdir(os.path.join(constVerwijzingDir, d))
]
for verwijzingsDir in opTePakkenVerwijzingDirs:
    verwijzingsFile = os.path.join(
        constVerwijzingDir, verwijzingsDir, "verwijzingen.txt"
    )
    lijstMislukteUrls = []
    ...
    tijdenVorigePunt = geeft_voortgangs_informatie(
        ...
        + verwijzingsDir
        + " met "
        + str(len(verwijzingen))
        + " verwijzingen. ",
        tijdenVorigePunt,
    )
    for verwijzing in verwijzingen:
        if not plaatje_gedownload(
            verwijzing, os.path.join(constVerwijzingDir, verwijzingsDir)
        ):
    ...
    if len(lijstMislukteUrls) > 0:
        write_lijst_regels_naar_file(
            os.path.join(constVerwijzingDir, verwijzingsDir, "foutGegaan.txt"),
            lijstMislukteUrls,
        )
    else:
        shutil.move(
            os.path.join(constVerwijzingDir, verwijzingsDir),
